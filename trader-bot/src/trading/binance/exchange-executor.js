...
export async function executeTrade(symbol, cfg, analysis, side, price) {
	...
	if (TRADE_MODE === 'paper') {
		console.log(`🟢 [PAPER] Simulated ${side} ${symbol} @ ${entryPrice} (size=${size}$, lev=${leverage}x)`);
		return pos;
	}
	...
	try {
		...
		console.error(`❌ Failed to set leverage for ${symbol}:`, err?.message || err);
	}
	...
	try {
		...
		console.error(`❌ Failed to fetch filters for ${symbol}:`, err?.message || err);
		return null;
	}
	...
	if (!qty || Number(qty) <= 0) {
		console.error(`❌ Quantity too small, skip trade ${symbol} (raw=${rawQty})`);
		return null;
	}
	...
	try {
		...
		console.warn(`⚠️ Failed to cancel existing orders for ${symbol}:`, err?.message || err);
	}
	...
	try {
		...
		console.error(`❌ Failed to open market order for ${symbol}:`, err?.message || err);
		return null;
	}
	...
	if (Array.isArray(takeProfits) && takeProfits.length) {
		for (const tp of takeProfits) {
			try {
				...
				console.warn(`⚠️ Failed to place TP for ${symbol}:`, err?.message || err);
			}
		}
	}
	...
	try {
		...
		if (live && Number(live.positionAmt) !== 0) {
			...
			pos = {
				...
				updates: [
					...(pos.updates || []),
					{ time: new Date().toISOString(), action: 'OPEN_CONFIRMED', price: avgEntry },
				],
			};
		}
	} catch (err) {
		console.warn(`⚠️ Failed to read live position for ${symbol}:`, err?.message || err);
	}
