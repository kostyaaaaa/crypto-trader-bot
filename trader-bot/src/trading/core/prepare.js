...
export async function preparePosition(symbol, cfg, analysis, side, entryPrice) {
  ...
  const marginUsd = (Number(capital.account) * Number(capital.riskPerTradePct)) / 100; // твоя маржа ($)
  ...
  if (exits?.sl?.type === 'atr') {
    ...
    if (Number.isFinite(atr) && atr > 0) {
      ...
      const lossPctOfMargin = marginUsd > 0 ? (approxLossUsd / marginUsd) * 100 : 0;
  ...
  if (exits?.tp?.use && Array.isArray(exits.tp.tpGridPct)) {
    ...
    for (let i = 0; i < exits.tp.tpGridPct.length; i++) {
      ...
      const tpPrice   = side === 'LONG' ? entryPrice + tpDist : entryPrice - tpDist;
