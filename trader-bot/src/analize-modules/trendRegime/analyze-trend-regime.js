...
export async function analyzeTrendRegime(symbol, candles, options = 14) {
  // options: number => period, or { period, adxSignalMin }
  const period =
  ...
  // --- сигнал ---
  // direction by DI, gating by ADX threshold for the *final* signal only
  let dir = 'NEUTRAL';
  ...
  if (lastAdx >= adxSignalMin && dir !== 'NEUTRAL') {
    signal = dir; // require minimum ADX to confirm signal
  }
